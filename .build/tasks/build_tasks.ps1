...
.DESCRIPTION
    bsdev is a Rust application that also ships a Docker container. These tasks therefore combine the
    RustApp native-binary pipeline (Build/CargoTest/Package/PublishRelease asset upload) with the
    container publish pipeline (BuildImage/push to GHCR and DockerHub). The container is independent
    of the compiled Rust binary; its build context is the 'image' directory.
#>
[CmdletBinding()]
param
(
    ...
    # Path (relative to the repository root) to the directory containing the Dockerfile and its build context.
    [Parameter(
        Mandatory = $false
    )]
    [string]
    $DockerContextPath = 'image'
)

# Docker requires image names to be lowercase
$ImageName = $ImageName.ToLower()

# The build context for the container is the 'image' directory (the Dockerfile lives there and COPYs
# files relative to it), not the repository root.
...
.DESCRIPTION
    Shared by the BuildImage task (CI smoke test) and the PublishRelease task. The build context is the
    'image' directory rather than the repository root.
#>
...
.DESCRIPTION
    Sets CARGO_TARGET_DIR to the build output directory so the compiled binary lands in .tmp/output.
#>
task Build {
...
<#
.SYNOPSIS
    Builds the Docker image (container smoke test / release artifact).
#>
task BuildImage {
    Invoke-DockerBuild
}
...
task Package GetReleaseHistory, Build, {
    ...
    # Cargo places the binary under $BrownserveRepoBuildOutputDirectory because CARGO_TARGET_DIR is
    # set by the Build task. The release subdirectory mirrors the normal target/[triple]/release layout.
    $BinDir = if ($Target)
    {
...
<#
.SYNOPSIS
    Publishes the release: creates a GitHub release with the platform binaries as assets and pushes the
    Docker image to the selected registries.
.DESCRIPTION
    ...
    A draft GitHub release is created first and only marked live as the final step, so that if anything
    fails during publishing (binary upload or container push) we are left with a mutable draft rather
    than a locked, published release.
#>
task PublishRelease GetReleaseHistory, {
    ...
    # Step 1: create a draft GitHub release and upload the platform binaries as assets.
    if ('GitHub' -in $PublishTo)
    {
    ...
    # Step 2: build the Docker image (only when a container registry is targeted).
    if (('DockerHub' -in $PublishTo) -or ('GHCR' -in $PublishTo))
    {
        Invoke-DockerBuild
    }
    ...
    # Step 4: push to GitHub Container Registry. The owner is lowercased because registry paths must be lowercase.
    if ('GHCR' -in $PublishTo)
    {
    ...
    # Step 5: all publishing complete - mark the GitHub release live as the final step.
    if (('GitHub' -in $PublishTo) -and $ReleaseResponse)
    {
...
<#
.SYNOPSIS
    Meta task: build the Rust app, run all Cargo tests and Pester smoke tests.
...
<#
.SYNOPSIS
    Meta task: performs all release steps except publishing.
    Useful for verifying the release process (including the container build) locally without pushing anything.
#>
task DryRun SetReleaseVariables, CheckPublishingParameters, GetReleaseHistory, Build, CargoTest, Tests, Package, BuildImage, {}

<#
.SYNOPSIS
    Meta task: creates a GitHub release (uploading all platform archives as assets) and pushes the
    Docker image to the selected registries.
